...
export const user = (state: UserState = initialUserState, action: Action): UserState => {
    switch(action.type) {
        case USER_TYPES.LOGIN_USER_START_ACTION: {
            return { ...state, isLoading: true };
        }
        case USER_TYPES.LOGIN_USER_SUCCESSFUL_ACTION: {
            return { isLoading: false, isLogged: true, user: action.payload };
        }
        case USER_TYPES.LOGOUT_USER_ACTION: {
            return { user: { id: null, name: "" }, isLogged: false, isLoading: false };
        }
        default:
            return state;
    }
